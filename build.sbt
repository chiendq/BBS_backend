...
lazy val port = (project in file("app/port"))
  .enablePlugins(PlayScala)
  .disablePlugins(PlayLayoutPlugin)
  .aggregate(portWebServices, portMySqlDatabase)
  .dependsOn(portWebServices, portMySqlDatabase)
  .settings(commonSettings)

lazy val portWebServices = (project in file("app/port/primary/webService"))
  .enablePlugins(PlayScala)
  .disablePlugins(PlayLayoutPlugin)
  .settings(commonSettings)

lazy val portMySqlDatabase = (project in file("app/port/secondary/mySqlDatabase"))
  .enablePlugins(PlayScala)
  .disablePlugins(PlayLayoutPlugin)
  .settings(commonSettings)